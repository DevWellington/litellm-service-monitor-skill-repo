#!/bin/bash
# LiteLLM Public Proxy Daemon Controller
# This script manages the public LiteLLM Proxy process (start, stop, status, logs).
...
show_help() {
    echo "LiteLLM Public Daemon - Controller"
    echo "Usage: $0 <action> [options]"
    echo ""
    echo "Actions:"
    echo "  start           Starts the public LiteLLM server"
    echo "  stop            Stops the running public LiteLLM server"
    echo "  status          Checks public server status and port"
    echo "  logs            Displays public server stdout and error logs"
    echo "  restart         Restarts the public LiteLLM server"
    echo ""
...
start_server() {
    ...
    if [ -f "$PID_FILE" ]; then
        ...
        if ps -p "$PID" >/dev/null 2>&1; then
            echo "LiteLLM Public já está rodando (PID: $PID)."
            exit 1
        fi
        rm -f "$PID_FILE"
    fi
    ...
    echo "Iniciando LiteLLM Public em http://${HOST}:${PORT}..."
    if [ "$FOREGROUND" = true ]; then
        ...
        if [ "$HEALTHY" = true ]; then
            ...
            echo "LiteLLM Public iniciado com sucesso! (PID: $BG_PID)"
            echo "Acesse em: http://${HOST}:${PORT}"
        else
            echo "Erro: LiteLLM Public falhou ao iniciar na porta $PORT."
...
stop_server() {
    if [ -f "$PID_FILE" ]; then
        PID=$(cat "$PID_FILE")
        if ps -p "$PID" > /dev/null 2>&1; then
            echo "Parando LiteLLM Public (PID: $PID)..."
            kill "$PID" 2>/dev/null
            sleep 1.5
            if ps -p "$PID" > /dev/null 2>&1; then
            ...
            echo "LiteLLM Public parado com sucesso."
        else
            echo "O processo com PID $PID não está ativo. Limpando arquivo de PID."
        fi
        rm -f "$PID_FILE"
    else
        echo "Nenhum servidor LiteLLM Public ativo encontrado."
    fi
}
...
show_logs() {
    if [ -f "$LOG_FILE" ]; then
        echo "=== LiteLLM Public Console Logs ($LOG_FILE) ==="
        FOLLOW=false
